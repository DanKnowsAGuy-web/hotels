...
(() => {
  ...
  /* ---------- hero: pinned scroll-scrub (annotations draw on, B&W -> colour) ----------
     Desktop + motion only. One --p (0..1) on .hero-scrolly is read by the CSS to draw the
     pen marks and colourise the hotel while the hero is pinned; then it releases. Reduced
     motion / small screens skip the pin and keep the static resting frame. */
  ...
  if (ledger) {
    ...
    /* hospitality-relevant first: a hotel reader should meet the same rooftop /
       compressor / HVAC technology before the rest of the record */
    const rank = (r) => {
      const s = r[1] + " " + r[2] + " " + r[3];
      if (/hotel|hospitality|lodging|resort|motel/i.test(s)) return 0;
      if (/hvac|rooftop|compressor|chiller|ptac|motor|office|building/i.test(s)) return 1;
      return 2;
    };
